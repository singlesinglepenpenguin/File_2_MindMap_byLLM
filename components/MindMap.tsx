...
// Initialize Mermaid configuration outside the component
mermaid.initialize({
  startOnLoad: true,
  theme: 'default',
  flowchart: {
...
export default function MindMap({ data }: Readonly<MindMapProps>) {
  ...
  const generateMermaidDiagram = (node: NodeData, level = 0): string => {
    console.log('Generating Mermaid diagram, node:', node);
    let diagram = '';
    ...
    try {
        ...
        if (node.details) {
            console.log('Processing node details:', node.details);
        ...
        if (node.children && node.children.length > 0) {
            console.log(`Processing ${node.children.length} child nodes`);
  ...
  useEffect(() => {
    console.log('MindMap useEffect triggered, data:', data);
    
    if (!data?.id) {
      console.error('Invalid mind map data received');
      return;
    }
    if (!mermaidRef.current) {
        console.error('mermaidRef not initialized');
        return;
    }

    const renderDiagram = async () => {
        try {
            const mermaidString = generateMermaidDiagram(data);
            ...
            const { svg } = await mermaid.render('mindmap-' + Date.now(), diagram);
            console.log('Mermaid rendering complete, SVG length:', svg.length);
    ...
    // 给 mermaid.js 一点时间初始化
    setTimeout(() => {
        console.log('Starting to clear old diagram');
        if (mermaidRef.current) {
            mermaidRef.current.innerHTML = '';
            setIsRendered(false);
            setZoomLevel(1);
            setPosition({ x: 0, y: 0 });
            console.log('Old diagram cleared');
        }
